...
def path_contains_tags(path, tags):
    """Returns True if the base name of a path contains all strings in tags, separated by underscores."""
    name = os.path.basename(path)
    tags_in_path = name.split(',')
    return set(tags).issubset(tags_in_path)


class ListWithNoSpaces(list):
    """Subclass of list so that string representation contains no spaces."""
    def __str__(self):
        s = super().__str__()
        return s.replace(' ', '')
...
class PyExperimentSuite(object):
    ...
    def parse_cfg(self):
        ...
        self.cfgparser.optionxform = str  # make case-sensitive
    ...
    def get_params(self, exp, cfgname='experiment.cfg'):
        ...
        """
        cfgp = ConfigParser()
        cfgp.optionxform = str  # make case-sensitive
    ...
    def get_values_fix_params(self, exp, rep, tag, which='last', **kwargs):
        ...
        desired_subexps = [se for se in subexps if path_contains_tags(se, tagvalues)]
        values = [self.get_value(se, rep, tag, which) for se in desired_subexps]
        params = [self.get_params(se) for se in desired_subexps]
            ...
            if ('experiment' in params and params['experiment'] == 'single'):
                ...
                if len(iterparams) > 0:
                    ...
                    for il in iterfunc(*[params[p] for p in iterparams]):
                        par = params.copy()
                        converted = list(zip(iterparams, map(convert_param_to_dirname, il)))
                        name = ','.join(['='.join(pair) for pair in sorted(converted)])
                        par['name'] = par['name'] + '/' + name
                        #converted = str(list(zip(iterparams, map(convert_param_to_dirname, il))))
