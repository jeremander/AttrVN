...
void mt_mergesort_r(void *arr, size_t num_elem, size_t elem_size, void *arg, Compare_fn f,
  ...
  tids = (pthread_t *)calloc((size_t)num_threads, sizeof(pthread_t));
  ...
  if (num_threads > 1) {
    ...
    for (i = 2; i < num_threads; i++) {
      combo = merge(combo, segments[i], combo_len, array_part[i].len, size, arg, f);
      free(temp);
      temp = combo;
