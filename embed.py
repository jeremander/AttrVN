"""This script reads in graph and attribute data, constructs similarity matrices for each attribute of interest, then embeds each of these into Euclidean space. 
...
def main():
    ...
    if pm.use_context:
        if (pm.load_embeddings and ('*context*_embedding.pickle' in filenames)):
            context_features = timeit(load_object, pm.verbose)(path, '*context*_embedding', 'pickle', verbose = pm.verbose)
        else:
            A = edgelist_to_sparse_adjacency_operator(edges_filename, verbose = pm.verbose)
            (eigvals, context_features) = timeit(embed_symmetric_operator, pm.verbose)(A, embedding = embedding, k = pm.max_eig, tol = None, verbose = pm.verbose)
            ...
            timeit(save_object, pm.verbose)(context_features, path, '*context*_embedding', 'pickle', verbose = pm.verbose)
            if pm.save_info:
    ...
    predictor_attr_types = [attr_type for attr_type in pm.predictor_attr_types if (attr_type != pm.nomination_attr_type)]
    embedding_filename_prefixes = ['%s_embedding_sim=%s_delta=%s_%s' % (attr_type, pm.sim, str(pm.delta), pm.embedding) for attr_type in predictor_attr_types]
    attr_features_by_type = dict()
    if pm.load_embeddings:  # first see what can be loaded from files
        for (attr_type, embedding_filename_prefix) in zip(predictor_attr_types, embedding_filename_prefixes):  
            ...
            if (len(valid_filenames) > 0):  # just use the first valid filename
                attr_features_by_type[attr_type] = timeit(load_object, pm.verbose)(path, valid_filenames[0][:-7], 'pickle', verbose = pm.verbose)
    if (len(attr_features_by_type) < len(predictor_attr_types)):  # need to construct AttributeAnalyzer
        if pm.verbose:
            print("\nCreating AttributeAnalyzer...")
        a = timeit(AttributeAnalyzer, pm.verbose)(attr_filename, context_features.shape[0], pm.predictor_attr_types)
        if pm.save_info:
    ...
    attr_types_to_embed = [attr_type for attr_type in predictor_attr_types if (attr_type not in attr_features_by_type)]
    for attr_type in attr_types_to_embed:
        ...
        attr_features_by_type[attr_type] = attr_features
    ...
    return (context_features, attr_features_by_type)
