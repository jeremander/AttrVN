...
class VNGraph(nx.Graph):
    ...
    def vn_supervised(self, k, embedding = 'adj', classifier = 'logreg', normalize = True, num_trees = 100, verbose = True):
        """Performs vertex nomination on a partially occluded graph by embedding the adjacency/Laplacian matrix and doing soft classification. k is number of features; embedding is 'adj' (adjacency A), 'adj+diag' (A + D/n), 'normlap' (normalized Laplacian D^(-1/2) * A * D^(-1/2)), or 'regnormlap' (regularized normalized Laplacian (D + tau * I)^(-1/2) * A * (D + tau * I)^(-1/2), for tau = mean(D)); classifier is 'logreg' (logistic regression), 'gnb' (Gaussian Naive Bayes), 'randfor' (random forest), or 'boost' (AdaBoost); normalize = True iff feature vectors are to be projected to sphere; num_trees is the number of trees to use if random forest or AdaBoost is used. Returns the vector of class probabilities corresponding to the unobserved nodes."""
        self.make_sparse_adjacency_operator()
    ...
    @classmethod
    def from_edgelist(cls, filename):
        g = cls()
        g.copy_edges_from_graph(nx.read_edgelist(filename))
        return g
