...
def compute_squared_distances(points, seeds, num_threads = -1):
    ...
    libbln.py_compute_squared_distances(n, s, dim, arr2D_to_ptr(points), arr2D_to_ptr(seeds), arr2D_to_ptr(D), num_threads)
    return D
...
def balloon_rank_from_distances(D, labels, deflate = False, num_threads = -1):
    ...
    D_copy = deepcopy(D)
    libbln.py_balloon_rank_from_distances(n, s, arr2D_to_ptr(D_copy), labels, ranks, deflate, num_threads)
    return ranks   
...
def balloon_rank(points, seeds, labels, deflate = False, num_threads = -1):
    ...
    ranks = np.zeros(n, dtype = int)
    libbln.py_balloon_rank(n, s, dim, arr2D_to_ptr(points), arr2D_to_ptr(seeds), labels, ranks, deflate, num_threads)
    return ranks   

class BalloonNominate():
    def __init__(self, lamb, deflate = False, n_jobs = -1, use_c = True):
    ...
    def predict_proba_py(self, X):
        """Given n x m matrix X, returns rank-derived scores for the n data points, using either the inflation or deflation method. (This is an older, slower Python version known to be accurate)."""
        ...
        return scores
    def predict_proba_c(self, X):
        """Given n x m matrix X, returns rank-derived scores for the n data points, using either the inflation or deflation method. (This uses a shared C library for efficiency)."""
        labels = np.asarray(self.labels, dtype = float)
        if (self.lamb <= 0.5):
            labels = (1 / (1 - self.lamb)) * labels - 1
        else:
            labels = (1 / self.lamb) * labels + ((self.lamb - 1) / self.lamb)
        ranks = balloon_rank(X, self.seeds, labels, self.deflate, num_threads = self.n_jobs)
        n = X.shape[0]
        scores = np.zeros((n, 2))
    ...
    def predict_proba(self, X):
        if self.use_c:
            try:  # use the C version if possible
                return self.predict_proba_c(X)
            except Exception as e:  # revert to Python
                print("ctypes error: %s\nUsing Python version..." % str(e))
                pass
        return self.predict_proba_py(X)
...
class TestBalloon(unittest.TestCase):
    ...
    def test_correctness(self, testnum, lamb = 0.5, deflate = False, use_c = True):
        (seeds, labels, X, true_ranked_list) = self.get_test_data(testnum)
        bln = BalloonNominate(lamb, deflate = deflate, use_c = use_c)
    ...
    def test_time(self, nseeds = 100, npoints = 1000, dim = 50, lamb = 0.5, deflate = False, n_jobs = -1, use_c = True):
        np.random.seed(0)
        seeds = np.random.randn(nseeds, dim)
        labels = np.random.randint(0, 2, nseeds)
        X = np.random.randn(npoints, dim)
        start = time.time()
        bln = BalloonNominate(lamb, deflate = deflate, n_jobs = n_jobs, use_c = use_c)
        bln.fit(seeds, labels)
        scores = bln.predict_proba(X)[:, 1]
        end = time.time()
        ranked_list = [pair[0] for pair in sorted(enumerate(scores), key = lambda pair : pair[1], reverse = True)]
